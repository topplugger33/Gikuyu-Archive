...
mongoose.connect(MONGO_URI)
    .then(() => { console.log("✅ Connected to MongoDB"); seedArticles(); seedVideos(); })
...
async function seedVideos() {
    ...
    if (count > 0) return;
    const seedVideos = [
        { title: "Welcome to Gĩkũyũ Archive", youtubeId: "dQw4w9WgXcQ", description: "An introduction to the archive and what we do.", category: "Introduction", gated: false, order: 1 },
        { title: "Introduction to Rũracio", youtubeId: "dQw4w9WgXcQ", description: "A brief overview of the traditional Kikuyu wedding process.", category: "Rũracio", gated: false, order: 2 },
        { title: "The Full Rũracio Ceremony", youtubeId: "dQw4w9WgXcQ", description: "Complete ceremony, step by step. Members only.", category: "Rũracio", gated: true, order: 3 },
        { title: "Elders Speak: Oral Histories Vol. 1", youtubeId: "dQw4w9WgXcQ", description: "Recorded wisdom from community elders.", category: "Oral History", gated: true, order: 4 },
        { title: "Traditional Kikuyu Songs & Dance", youtubeId: "dQw4w9WgXcQ", description: "Live recording of traditional performances.", category: "Music", gated: true, order: 5 },
        { title: "Kikuyu Language Basics", youtubeId: "dQw4w9WgXcQ", description: "Learn foundational Kikuyu phrases and greetings.", category: "Language", gated: false, order: 6 }
    ];
...
app.get("/api/videos", async (req, res) => {
    try {
        const videos = await Video.find().sort({ order: 1, date: -1 });
        let isLoggedIn = false;
        const authHeader = req.headers["authorization"];
        const token = authHeader && authHeader.split(" ")[1];
        if (token) { try { jwt.verify(token, JWT_SECRET); isLoggedIn = true; } catch (e) {} }
        const result = videos.map(v => ({
            _id: v._id,
            title: v.title,
            youtubeId: (!v.gated || isLoggedIn) ? v.youtubeId : null,
            description: v.description,
            category: v.category,
            gated: v.gated,
            locked: v.gated && !isLoggedIn,
            order: v.order
        }));
        res.json({ success: true, videos: result, isLoggedIn });
    } catch (e) { res.status(500).json({ success: false, message: "Server error." }); }
});

app.get("/api/admin/stats", authenticateToken, isAdmin, async (req, res) => {
    try {
        const [messageCount, userCount, articleCount, commentCount, likeCount, pendingCount, postCount, videoCount] = await Promise.all([
            ...
            Article.countDocuments({ status: "pending" }), Post.countDocuments(), Video.countDocuments()
        ]);
        res.json({ success: true, stats: { messageCount, userCount, articleCount, commentCount, likeCount, pendingCount, postCount, videoCount } });
    } catch (e) { res.status(500).json({ success: false, message: "Server error." }); }
});
...
app.get("/api/admin/videos", authenticateToken, isAdmin, async (req, res) => {
    try { res.json({ success: true, videos: await Video.find().sort({ order: 1, date: -1 }) }); }
    catch (e) { res.status(500).json({ success: false, message: "Server error." }); }
});
app.post("/api/admin/videos", authenticateToken, isAdmin, async (req, res) => {
    try {
        const { title, youtubeId, description, category, gated, order } = req.body;
        if (!title || !youtubeId) return res.status(400).json({ success: false, message: "Title and YouTube ID required." });
...
app.put("/api/admin/videos/:id", authenticateToken, isAdmin, async (req, res) => {
    try {
        const update = { ...req.body };
        if (update.gated !== undefined) update.gated = update.gated === true || update.gated === "true";
...
app.delete("/api/admin/videos/:id", authenticateToken, isAdmin, async (req, res) => {
    try { await Video.findByIdAndDelete(req.params.id); res.json({ success: true, message: "Video deleted." }); }
    catch (e) { res.status(500).json({ success: false, message: "Server error." }); }
});
